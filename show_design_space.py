...
def main():
    ...
    CS = plt.contour(X1, X2, F2, levels, linestyles="dashed", colors="black",
                     alpha=0.5)
    CS.collections[0].set_label("$f_2(x)$")

    plt.plot(X1[0], G1, linewidth=2.0, color="green", linestyle="dotted")
    plt.plot(X1[0][G1<0], G1[G1<0], label="$g_1(x)$", linewidth=2.0,
             color="green")
    ...
    plt.plot(np.linspace(0.1, 0.4, 100), np.zeros(100), linewidth=3.0,
             color="orange")
    plt.plot(np.linspace(0.6, 0.9, 100), np.zeros(100), linewidth=3.0,
             color="orange")
